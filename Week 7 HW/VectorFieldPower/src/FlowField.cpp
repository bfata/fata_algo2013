...
//Set all the forces in the field based on Mouse Position
void FlowField::setStrange(){
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            float screenX = (ofGetWidth());
            float screenY = (ofGetHeight());
            
            float arrowX = (ofGetMouseX()/ screenX);
            float arrowY = (ofGetMouseY()/ screenY);
//            ofLog() << arrowX;
            flowList[y][x].set( ofVec2f( arrowX, arrowY));
//            
//            if (ofGetMouseX() > ofGetScreenWidth()/2){
//                arrowX = arrowX *-1;
//            }
//            if (ofGetMouseY() < ofGetScreenHeight()/2){
//                arrowY= arrowY *-1;
//            }
